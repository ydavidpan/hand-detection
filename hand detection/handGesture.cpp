...
float HandGesture::distanceP2P(Point a, Point b){
	float d= sqrt(fabs( pow((double)(a.x-b.x),2.0) + pow((double)(a.y-b.y),2) )) ;  
	return d;
}
...
void HandGesture::computeFingerNumber(){
  ...
  //  Matchnum = mostFrequentFingerNumber;
}
...
void HandGesture::checkForOneFinger(MyImage *m){
	...
	while( d!=hullP[cIdx].end() ) {
        ...
        if((v.y>(m->src.rows)/2 + 150)&&(v.x<(m->src.cols)/2 + 150)&&(v.x>(m->src.cols)/2 - 150))
        {
            cout<<"up"<<endl;
            key = 222;
        }
            ...
            if((v.y<(m->src.rows)/2 - 150)&&(v.x<(m->src.cols)/2 + 150)&&(v.x>(m->src.cols)/2 - 150))
            {
                ...
                if((v.x>(m->src.cols)/2 + 200)&&(v.y<(m->src.rows)/2 + 150))
                {
                    cout<<"right"<<endl;
